# --
# Kernel/Modules/AdminDynamicFieldCustomText.pm - provides a dynamic fields text config view for admins
# Copyright (C) 2013 Mike McMahon, http://mikemcmahon.github.io
# --
# $Id: AdminDynamicFieldCustomText.pm,v 1.32 2012/11/20 14:37:27 mh Exp $
# --
...
package Kernel::Modules::AdminDynamicFieldCustomText;
...
sub _AddAction {
    ...
    for my $ConfigParam (
        qw(ObjectType ObjectTypeName FieldType FieldTypeName DefaultValue ValidID Link RegexValue AutocompleteType)
        )
    {
    ...
    if ( $GetParam{FieldType} eq 'RegexText' ) {
        $FieldConfig->{RegexValue} = $GetParam{RegexValue}; 
    }
    elsif ( $GetParam{FieldType} eq 'AutocompleteText' ) { 
        $FieldConfig->{AutocompleteType} = $GetParam{AutocompleteType};
    }
...
sub _ChangeAction {
    ...
    for my $ConfigParam (
        qw(ObjectType ObjectTypeName FieldType FieldTypeName DefaultValue ValidID RegexValue AutocompleteType Link)
        )
    {
        $GetParam{$ConfigParam} = $Self->{ParamObject}->GetParam( Param => $ConfigParam );
    }
...
sub _ShowScreen {
    ...
    if ( $Param{FieldType} eq 'RegexText' ) {
        # define config field specific settings
        my $RegexValue = ( defined $Param{RegexValue} ? $Param{RegexValue} : '' );

        # create the regex element
        $Self->{LayoutObject}->Block(
            Name => 'RegexValue',
            Data => {
                %Param,
    ...
    elsif ( $Param{FieldType} eq 'AutocompleteText' ) { 
        my $SelectedValue = ( defined $Param{AutocompleteType} ? $Param{AutocompleteType} : ''); 

        my $AutocompleteLookupType = $Self->{LayoutObject}->BuildSelection(
            Data            => {
                Agent => 'Agent', 
                Customer => 'Customer'
            },
        ...
        $Self->{LayoutObject}->Block(
            ...
            Data => { 
                %Param,
                AutocompleteType => $AutocompleteLookupType,
            },
        );
    }
    ...
    $Output .= $Self->{LayoutObject}->Output(
        TemplateFile => 'AdminDynamicFieldCustomText',
        Data         => {
            %Param,
