...
package Kernel::System::DynamicField::Backend::RegexText;
...
Kernel::System::DynamicField::Backend::RegexText
...
sub EditFieldValueValidate {
    ...
    # match the regex from the config
    my $Regex = $Param{DynamicFieldConfig}->{Config}->{RegexValue};
    if ( !($Value =~ m/$Regex/) ) {
        $ServerError = 1;
        $ErrorMessage = "Your value must match $Regex"; 
    }
